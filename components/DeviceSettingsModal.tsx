...
const FREQ_PRESETS = [
  ...
  { label: '869.618 MHz (EU MeshCore)', hz: 869_618_000 },
...
export default function DeviceSettingsModal({ visible, onClose }: DeviceSettingsModalProps) {
  ...
  // Radio (defaults: EU MeshCore Narrow — 869.618 MHz / 62.5 kHz / SF8 / CR4/8)
  const [freqHz, setFreqHz] = useState(869_618_000);
  const [bwHz, setBwHz] = useState(62_500);
  ...
  useEffect(() => {
    if (!info) return;
    setFreqHz(info.radioFreqHz || 869_618_000);
    setBwHz(info.radioBwHz || 62_500);
    setSf(info.radioSf || 8);
    setCr(info.radioCr || 8);
