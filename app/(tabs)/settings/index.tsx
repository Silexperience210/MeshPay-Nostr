        ...
        <View style={styles.sectionCard}>
          <SettingRow
            ...
            onPress={() => {
              ...
              const bands = [433_175_000, 868_000_000, 869_525_000, 869_618_000, 915_000_000];
              Alert.alert(
                'Fréquence LoRa',
                `Actuel : ${(bleDeviceInfo.radioFreqHz / 1e6).toFixed(3)} MHz`,
                bands.map((hz) => ({
                  text: `${(hz / 1e6).toFixed(3)} MHz`,
                  onPress: () => setRadioParams(hz, bleDeviceInfo.radioBwHz, bleDeviceInfo.radioSf, bleDeviceInfo.radioCr || 8)
            ...
            onPress={() => {
              ...
              Alert.alert(
                ...
                [7, 8, 9, 10, 11, 12].map((sf) => ({
                  text: `SF${sf}`,
                  onPress: () => setRadioParams(bleDeviceInfo.radioFreqHz, bleDeviceInfo.radioBwHz, sf, bleDeviceInfo.radioCr || 8)
                    .catch((e: any) => Alert.alert('Erreur', e.message)),
                })).concat([{ text: 'Annuler', onPress: async () => {} }]),
              );
            }}
          ...
          />
          <SettingRow
            icon={<SettingsIcon size={18} color={btEnabled ? Colors.purple : Colors.textMuted} />}
            label="Configuration avancée"
            value={btEnabled ? 'Radio • Nom • GPS • Flood' : 'Non connecté'}
            onPress={() => {
              if (!btEnabled) { showAlert('Configuration avancée', 'Connectez un gateway BLE pour accéder à la configuration complète.'); return; }
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
      ...
      <View style={styles.section}>
        ...
        <View style={styles.sectionCard}>
          ...
          <SettingRow
            ...
            value={`v${Constants.expoConfig?.version ?? '1.0.8'}`}
            onPress={() => {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              Alert.alert(
                'MeshPay-Nostr',
                `Version : ${Constants.expoConfig?.version ?? '1.0.8'}\n` +
                `Protocole : MeshCore Companion v1.15.0\n` +
                `Off-grid messaging via LoRa + Nostr bridge.`,
