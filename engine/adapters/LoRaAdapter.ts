...
export class LoRaAdapter implements ProtocolAdapter {
  ...
  private setupListeners(): void {
    ...
    // onIncomingMessage est multi-listener et retourne un unsubscribe — on le
    // stocke pour cleanup propre dans stop(). (Avant 2026-05, l'API single-slot
    // forçait à passer un no-op qui n'effaçait rien et écrasait les autres
    // consommateurs comme BleProvider.)
    const unsubMsg = this.bleClient.onIncomingMessage((msg) => {
    ...
    this.unsubCallbacks.push(unsubMsg);
    this.unsubCallbacks.push(() => {
      // onContactDiscovered/onContacts restent single-slot — pas de fuite si on
      // les laisse pointer vers ce no-op au démontage de l'adapter.
