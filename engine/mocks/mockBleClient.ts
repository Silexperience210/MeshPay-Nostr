...
export const createMockBleClient = (): MockBleClient => {
  const mockClient = {
    ...
    // Écoute — onIncomingMessage est multi-listener (cf. ble-gateway.ts) et
    // retourne un unsubscribe. Le mock reproduit ce contrat pour LoRaAdapter.
    onIncomingMessage: jest.fn().mockImplementation((callback: (msg: MeshCoreIncomingMsg) => void) => {
      mockClient._incomingMessageCallbacks.push(callback);
      return () => {
        const idx = mockClient._incomingMessageCallbacks.indexOf(callback);
        if (idx >= 0) mockClient._incomingMessageCallbacks.splice(idx, 1);
      };
    }),
