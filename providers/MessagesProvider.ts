...
export const [MessagesContext, useMessages] = createContextHook((): MessagesState => {
  ...
  // subscribeDMsSealed(). Les messages arrivent dans MessagesProvider via le
  // messagingBus.subscribe() handler à la ligne ~1695 (Bridge Nostr → conversations).
  //
  // Si les DMs ne fonctionnent pas, vérifier que:
  //   1. NostrProvider.publicKey est non-null (identité Nostr dérivée)
  //   2. messagingBus.setLocalIdentity() a été appelé (MessagingBusProvider)
  //   3. messagingBus._startNostrListeners() a démarré (handlers.size > 0)
