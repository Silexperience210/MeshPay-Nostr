...
export class BleGatewayClient {
  ...
  // Multi-listener : plusieurs consommateurs (BleProvider + LoRaAdapter) doivent
  // pouvoir s'abonner aux messages parsés sans s'écraser mutuellement (cf. bug
  // 2026-05 : LoRaAdapter écrasait silencieusement la subscription de BleProvider).
  private incomingMessageCallbacks: Set<(msg: MeshCoreIncomingMsg) => void> = new Set();
  ...
  async sendDirectMessage(pubkeyHex: string, text: string, attempt = 0, localMsgId?: string): Promise<void> {
    ...
    // un chunking (ChunkManager) ou un message d'erreur clair à l'utilisateur.
  ...
  /**
   * Abonne un listener aux messages texte parsés (DM/canal/announce). Plusieurs
   * abonnés peuvent coexister — la fonction retourne un unsubscribe à appeler
   * pour retirer ce listener spécifique. (Avant 2026-05, l'API était single-slot
   * et un second appel écrasait le premier — cf. conflit BleProvider/LoRaAdapter.)
   */
  onIncomingMessage(cb: (msg: MeshCoreIncomingMsg) => void): () => void {
    this.incomingMessageCallbacks.add(cb);
    return () => { this.incomingMessageCallbacks.delete(cb); };
  }
  ...
  // ── Privé : multi-listener fan-out ─────────────────────────────────
  private emitIncomingMessage(msg: MeshCoreIncomingMsg): void {
    for (const cb of this.incomingMessageCallbacks) {
      try { cb(msg); } catch (e) {
        console.error('[BleGateway] incomingMessage listener error:', e);
      }
    }
  }
